...
| Model | Dimensions | Task Type | Description |
|-------|------------|-----------|-------------|
| text-embedding-005 | 768 | None | Google's general-purpose embedding |
| gemini-embedding-001 | 768 | SEMANTIC_SIMILARITY | Gemini with semantic similarity task |
| gemini-embedding-001 | 768 | RETRIEVAL_QUERY | Gemini with retrieval query task |
...
- **Reranker:** Vertex AI Ranking API (tested with and without)

## Data Files

- `embedding_comparison_recall_only_20251214_185916.json` - Main results (recall only)
- `embedding_comparison_rerank_20251214_175403.json` - Results with reranker
- `checkpoint_*.json` - Intermediate checkpoints
- `*_intermediate.json` - Per-model intermediate results
